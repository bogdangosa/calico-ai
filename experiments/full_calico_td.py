...
# Assuming these files exist in your project structure as indicated
from enviroment.calico_scoring import get_total_score_on_board
from utils.constants import *


# --- 1. REIMPLEMENTED CALICO ENV (With CNN Helpers) ---
# I will wrap your existing class structure but add the critical CNN state method.
...
class CalicoEnv:
    ...
    # ... [PASTING YOUR PROVIDED METHODS] ...
    def get_legal_actions(self):
        legal_actions = []
        if self.mode == "placing":
            # Optimization for full board: only check neighbors of existing tiles
            # to reduce branching factor from ~75 to ~10-15
            inner_board = self.board_matrix[1:BOARD_SIZE - 1, 1:BOARD_SIZE - 1]
            has_tiles = np.any(inner_board != NO_TILE_VALUE)

            empty_cells = []
            if not has_tiles:
                # If board empty, force center or objectives (heuristic start)
                # Center of 7x7 is (3,3)
                empty_cells = [(3, 3)]
                # Add objectives as valid starts
    ...
    # --- CNN REPRESENTATION FOR FULL GAME ---
    def get_cnn_state(self):
        """
        Returns (7, 7, 14) tensor.
        Channels:
          0-5: Colors (One Hot)
          6-11: Patterns (One Hot)
          12: Occupied
          13: Objective
        """
        H, W = self.board_matrix.shape
        # TILE_COLORS=6, TILE_PATTERNS=6 -> 12 channels + 2 = 14
        channels = TILE_COLORS + TILE_PATTERNS + 2
        state = np.zeros((H, W, channels), dtype=float)

        for r in range(H):
            for c in range(W):
                val = self.board_matrix[r][c]

                if val >= 0:  # It's a tile (0-35) or border
                    color = val // TILE_PATTERNS
                    pattern = val % TILE_PATTERNS

                    # Safety check for indices
                    if color < TILE_COLORS:
                        state[r, c, color] = 1.0
                    if pattern < TILE_PATTERNS:
...
# ==============================================================================
# PART 3: CONVOLUTIONAL NEURAL NETWORK (Scaled Up)
# ==============================================================================
...
class Conv2DLayer:
    def __init__(self, num_filters, filter_size, input_channels, lr=0.01):
        ...
        self.input_channels = input_channels  # Stored for compatibility check
        self.lr = lr
    ...
    def forward(self, X):
        ...
        # Vectorized approach for 3x3 filters
        # We extract all 3x3 patches into a matrix and do dot product
        # But keeping the loop for readability/stability in pure python context
...
class FullCNNValueNetwork:
    def __init__(self, board_size, channels, lr=0.005):
        # SCALED UP: 32 filters, larger Dense layer
        self.conv = Conv2DLayer(num_filters=32, filter_size=3, input_channels=channels, lr=lr)

        conv_out_dim = board_size - 2  # 7x7 -> 5x5
    ...
    def save(self, fname="full_cnn.pkl"):
        with open(fname, 'wb') as f:
            pickle.dump({'c': self.conv.filters, 'cb': self.conv.bias,
                         'w1': self.W1, 'b1': self.b1, 'w2': self.W2, 'b2': self.b2,
                         'ch': self.conv.input_channels}, f)

    def load(self, fname="full_cnn.pkl"):
        if os.path.exists(fname):
            try:
                with open(fname, 'rb') as f:
                    d = pickle.load(f)
                    if d.get('ch', 0) != self.conv.input_channels: return False
...
# ==============================================================================
# PART 4: FULL TD AGENT
# ==============================================================================

class FullTDAgent:
    def __init__(self, env, gamma=0.95, epsilon=1.0, decay=0.9970):
        self.env = env
        # 6 Colors + 6 Patterns + Occupied + Objective = 14 Channels
        ...
        self.decay = decay
        self.memory = deque(maxlen=5000)  # Larger memory for complex game
        self.batch_size = 32

    def get_state(self, env):
        return env.get_cnn_state()
    ...
    def get_best_action(self, env):
        ...
        # To speed up, maybe don't check ALL moves if > 20?
        # For now, check all to ensure optimality
        for act in legal:
    ...
    def replay(self):
        ...
        for s, a, r, ns, done in batch:
            if done:
                ...
                # Bootstrapping
                target = r + self.gamma * self.vn.forward(ns)
    ...
    def train_episode(self):
        ...
        # Score tracking
        # Note: get_total_score is computationally expensive to run every step.
        # We assume calculate_score is lighter or we use a delta proxy.
        # For full game, we rely on get_total_score_on_board
        prev_score = get_total_score_on_board(self.env.board_matrix, self.env.cat_tiles)

        init_pred = self.vn.forward(curr_state)
        done = False
        loss_sum = 0
        steps = 0
        ...
        return prev_score, loss_sum / max(1, steps), init_pred
...
if __name__ == "__main__":
    ...
    if agent.vn.load("full_calico_new_td.pkl"):
        print("Loaded weights.")
        agent.epsilon = 0.5

    print("Starting Full Calico Training (10k Episodes)...")
    episodes = 1000
    scores = []
    ...
    for e in range(episodes):
        ...
        if e % 10 == 0:
            ...
            print(f"Ep {e} | Score: {s} | Avg: {avg:.2f} | Eps: {agent.epsilon:.2f}")
